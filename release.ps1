<#
.SYNOPSIS
  Build and publish the blackduck-remediation-metrics package, with version bumping and optional install verification.
...
.PARAMETER GitHubRepo
  GitHub repository in owner/repo format. Default: lejouni/blackduck_remediation_metrics
...
param(
  ...
  [string]$GitHubRepo = 'lejouni/blackduck_remediation_metrics',
  [string]$GitHubToken,
  [switch]$SkipGitTag,
  [string]$TagPrefix = 'v',
...
function Set-ProjectVersion {
  ...
  $initPath = Join-Path (Split-Path $PyProjectPath) 'src/blackduck_remediation_metrics/__init__.py'
  ...
  # Also update __version__ in blackduck_triage_extract.py
  $extractPath = Join-Path (Split-Path $PyProjectPath) 'src/blackduck_remediation_metrics/blackduck_triage_extract.py'
  if (Test-Path $extractPath) {
    $extractContent = Get-Content -Raw -LiteralPath $extractPath
    $extractNew = $extractContent -replace '__version__\s*=\s*"\d+\.\d+\.\d+"', "__version__ = `"$Version`""
    if ($DryRun) {
      Write-Host "[DRY-RUN] Would set __version__ to $Version in $extractPath (UTF-8 no BOM)" -ForegroundColor Yellow
    } else {
      [System.IO.File]::WriteAllText($extractPath, $extractNew, $utf8NoBom)
      Write-Host "Set __version__ to $Version in $extractPath" -ForegroundColor Green
    }
  }
}
...
function New-GitHubRelease {
  ...
  $headers = @{
    ...
    'User-Agent'  = 'blackduck-remediation-release-script'
  }
  # Build JSON payload
...
if (-not $SkipBuild) {
  ...
  Get-ChildItem -Filter '*.egg-info' -Recurse | ForEach-Object {
...
try {
  if ($CreateGitHubRelease) {
    ...
    if ($ReleaseNotesPath -and (Test-Path $ReleaseNotesPath)) {
      ...
      $relBody = "Release $tag`n`nPublished to $Repository as blackduck-remediation-metrics $nextVersion."
    }
...
if ((-not $NoInstallTest) -and (-not $DryRun)) {
  ...
  if (-not $NoUpload) {
    ...
    $interval = 5
    $waited = 0
    $pypiUrl = "https://pypi.org/pypi/blackduck-remediation-metrics/json"
  ...
  if ($NoUpload) {
    ...
    if ($Repository -eq 'testpypi') {
      $installCmd = "$testPy -m pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple blackduck-remediation-metrics==$nextVersion"
    } else {
      $installCmd = "$testPy -m pip install --index-url https://pypi.org/simple/ blackduck-remediation-metrics==$nextVersion"
    }
  ...
  # Verify CLI by running the module
  Invoke-Step -Command "$testPy -m blackduck_remediation_metrics --help"
}
