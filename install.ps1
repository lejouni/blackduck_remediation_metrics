# Installation script for Black Duck Remediation Metrics
...
if ($Help) {
    Write-Host ""
    Write-Info "Black Duck Remediation Metrics - Installation Script"
    Write-Host ""
...
if ($Clean) {
    ...
    $cleanDirs = @(
        ...
        "dist",
        "*.egg-info",
        "src/blackduck_remediation_metrics.egg-info",
        "__pycache__",
        "src/__pycache__",
        "src/blackduck_remediation_metrics/__pycache__"
    )
...
try {
    $bdRemediationVersion = python -m blackduck_remediation_metrics --help 2>&1
    Write-Success "[OK] blackduck-remediation-metrics is available"
} catch {
    Write-Warning "[WARNING] Could not verify blackduck-remediation-metrics command. You may need to restart your terminal."
}

# Also try the short command alias
try {
    $bdMetricsVersion = bd-metrics --help 2>&1 | Select-Object -First 1
    Write-Success "[OK] bd-metrics command is available"
} catch {
    Write-Warning "[WARNING] bd-metrics command not found. You may need to restart your terminal."
}

Write-Host ""
Write-Info "Quick Start:"
Write-Host "  blackduck-remediation-metrics --token <TOKEN> --url <BD_URL> --html"
Write-Host "  bd-metrics --token <TOKEN> --url <BD_URL> --html"
Write-Host "  python -m blackduck_remediation_metrics --help"
Write-Host ""
...
Write-Info "For more information, see README.md or QUICKSTART.md"
Write-Host ""
